...
int main(){
	...
	scanf("%d",&n);
	float x[99][n+2],v[99][n+2],k[4][n+2],l[4][n+2],mx[99],mv[99],E[99][n+2];
	...
	for(j=0;j<99;j++){
		for(i=1;i<=n;i++){
			E[j][i]=(0.5*pow(v[j][i],2)) + (2.5*(pow((x[j][i]-x[j][i-1]),2.0)+pow((x[j][i]-x[j][i-1]),2.0)));
		}
	}
	for(j=0;j<99;j++){
		printf("%f",E[j][1]);
